...
(() => {
  ...
  function createCinemanPanel() {
    ...
    // Re-using the same panel structure and styling
    panel.innerHTML = `
  ...
  async function handleAddToWatchlist(event) {
    ...
    const posterEl = document.querySelector('.thumbnail-img');
    const summaryEl = document.querySelector('.description');
  ...
  function injectPanel() {
    ...
    if (!detailContainer || document.getElementById(CINEMAN_PANEL_ID)) return;
    
    const titleNode = detailContainer.querySelector('h1.content-title, .detail-page-title');
    if (!titleNode) {
      console.log("CineMan AI: Hotstar title node not found.");
      return;
    }
    ...
    const targetNode = detailContainer.querySelector('.action-buttons-container, .cta-container');
    if (targetNode) {
      const cinemanPanel = createCinemanPanel();
      // Hotstar layout might require insertion before or after. after seems safer.
      targetNode.insertAdjacentElement('afterend', cinemanPanel);
      console.log(`CineMan AI: Injected panel for "${currentTitle}" on Hotstar.`);
    } else {
      console.log("CineMan AI: Hotstar target node for injection not found.");
    }
  }

  const observer = new MutationObserver(() => {
    // Hotstar might change its main content container class, but this is a reasonable guess
    if (document.querySelector('.content-container') && !document.getElementById(CINEMAN_PANEL_ID)) {
      injectPanel();
    }
  });
