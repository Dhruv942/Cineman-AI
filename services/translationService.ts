...
export const translateUI = async (
  ...
  const targetLanguage = SUPPORTED_TRANSLATION_LANGUAGES.find(
  ...
  const prompt = `
        ...
        Your task is to translate a JSON object of English UI text strings into ${
          targetLanguage.name
        }.
  ...
  try {
    ...
    if (
      typeof translatedObject.appName !== "string" ||
      typeof translatedObject.landing_cta !== "string"
    ) {
