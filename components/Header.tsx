...
export const Header: React.FC<HeaderProps> = ({ onEditPreferences, onShowMyAccount, onShowOtherSettings }) => {
  ...
  return (
    <header className={`bg-slate-900/50 backdrop-blur-md shadow-lg sticky top-0 z-50 transition-all duration-300 ease-in-out ${isScrolled ? 'py-2' : 'py-3 sm:py-5'}`}>
      <div className="container mx-auto px-3 sm:px-4 relative flex items-center justify-between min-h-[60px] sm:h-16">
        
        {/* Scrolled Header Content (left-aligned) */}
        <div className={`flex items-center space-x-2 transition-opacity duration-300 ${isScrolled ? 'opacity-100' : 'opacity-0'}`}>
          <img
        ...
        <div
          className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 
            ...
            ${isScrolled ? 'opacity-0 invisible' : 'opacity-100'} w-full max-w-[calc(100%-80px)] sm:max-w-none`}
        >
          ...
          <p className="text-center text-xs sm:text-sm md:text-base lg:text-lg font-medium text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-teal-300 tracking-wide px-2 max-w-[280px] sm:max-w-xs md:max-w-md leading-tight">
        ...
        <div className="relative flex-shrink-0" ref={dropdownRef}>
          <button 
            ...
            className="p-1.5 sm:p-2 rounded-full hover:bg-slate-700/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-400 transition-colors"
          ...
          >
            <svg 
              ...
              strokeWidth={1.5} 
              stroke="currentColor" 
              className="w-6 h-6 sm:w-7 sm:h-7 text-slate-400 hover:text-purple-300 transition-colors"
            >
