...
export const LandingPage: React.FC<LandingPageProps> = ({ onStartOnboarding }) => {
  ...
  const FeatureCard: React.FC<{ icon: string; title: string; children: React.ReactNode }> = ({ icon, title, children }) => {
    ...
    return (
      <div className="bg-slate-800/50 backdrop-blur-sm p-3 sm:p-4 md:p-6 rounded-lg shadow-lg text-center transform hover:-translate-y-1 transition-transform duration-200">
        <div
          className="flex items-center justify-center mb-2 sm:mb-3 w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full bg-purple-500/20 mx-auto"
          dangerouslySetInnerHTML={{ __html: icon }}
        />
        <h3 className="text-sm sm:text-base md:text-lg font-semibold mb-1 sm:mb-2 text-white">{title}</h3>
        <p className="text-xs sm:text-sm md:text-base text-slate-300 leading-relaxed">{children}</p>
      </div>
    );
  };

  return (
    <div style={appStyle} className="min-h-screen w-full flex flex-col px-4 py-6 sm:py-8 text-slate-100 text-center">
      {/* Hero Section */}
      <div className="flex-shrink-0 pt-8 sm:pt-12 pb-6 sm:pb-8" style={{ animation: 'fade-in-hero 0.8s ease-out forwards' }}>
        <div className="flex flex-col sm:flex-row items-center justify-center space-y-2 sm:space-y-0 sm:space-x-3 mb-4 sm:mb-6">
          ...
          />
          <h1 className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500">
            CineMan AI
          </h1>
        </div>
        
        <p className="max-w-xl mx-auto text-base sm:text-xl md:text-2xl font-semibold text-slate-200 mb-3 sm:mb-4 px-2">
          Stop Scrolling, Start Watching.
        </p>
        
        <p className="max-w-2xl mx-auto text-sm sm:text-base md:text-lg text-slate-300 mb-6 sm:mb-8 px-2 leading-relaxed">
        ...
        <button
          ...
          className="w-full max-w-sm sm:w-auto px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold rounded-lg shadow-lg hover:shadow-xl transition-all duration-200 ease-in-out transform hover:scale-105 text-base sm:text-lg mx-auto"
        >
      ...
      {/* Features Section */}
      <div className="flex-grow py-6 sm:py-8" style={{ animation: 'fade-in-hero 0.8s ease-out 0.3s forwards' }}>
      ...
      {/* Footer */}
      <footer className="flex-shrink-0 py-4 sm:py-6 text-center w-full">
